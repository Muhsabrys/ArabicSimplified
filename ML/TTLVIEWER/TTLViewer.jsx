...
const NodeLinkVisualization = ({ triples }) => {
  ...
  useEffect(() => {
    ...
    svg.attr('viewBox', `0 0 ${width} ${height}`).call(d3.zoom().on("zoom", e => g.attr("transform", e.transform)));
    ...
    const tooltip = d3.select(svgRef.current.parentElement).append("div")
      .style("position","absolute").style("padding","4px 8px").style("background","#111")
      .style("color","#fff").style("border-radius","4px").style("display","none");
        ...
        <div>
          ...
          {activeTab==='triples' && (
            <div className="mt-2 max-h-96 overflow-y-scroll">
              {triples.map(t=><div key={t.id} className="text-xs border-b border-gray-700 py-1"><span className="text-blue-300">{t.s}</span> <span className="text-yellow-300">{t.p}</span> <span className="text-green-300">{t.o}</span></div>)}
            </div>
          )}
          {activeTab==='sparql' && (
            <div className="mt-2">
